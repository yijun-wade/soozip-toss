...
__export(index_exports, {
  ...
  getReactCompatibilityByReactNativeVersion: () => getReactCompatibilityByReactNativeVersion,
  getTossReactNativePackages: () => getTossReactNativePackages,
  reactCompatibilityPlugin: () => reactCompatibilityPlugin,
...
function replaceModulePath(path7, from, to) {
  const subpath = path7.slice(from.length);
  return `${to}${subpath}`;
}
...
function ensureAsyncStorageCompatFile(rootDir) {
  const graniteDirectoryPath = import_path3.default.join(rootDir, GRANITE_DIRECTORY_NAME);
  const asyncStorageCompatPath = import_path3.default.join(graniteDirectoryPath, ASYNC_STORAGE_COMPAT_FILE_NAME);
  try {
    import_fs.default.mkdirSync(graniteDirectoryPath, { recursive: true });
    import_fs.default.writeFileSync(asyncStorageCompatPath, createAsyncStorageCompatContent(), "utf8");
  } catch (error) {
...
function reactNativeCompatibilityPlugin(options) {
  ...
  if (isReactNativeVersionOverridden) {
    compatibilityReactNativePath = import_path4.default.dirname(require.resolve(`${compatibilityReactNativePackage}/package.json`));
  }
...
function ensureReverseCompatFiles({ rootDir, isHost }) {
  const graniteDirectoryPath = import_path5.default.join(rootDir, GRANITE_DIRECTORY_NAME2);
  const brickModuleCompatPath = import_path5.default.join(graniteDirectoryPath, BRICK_MODULE_COMPAT_FILE_NAME);
  try {
    const reverseBrickModuleCompatContent = createReverseBrickModuleCompatContent({ isHost });
    import_fs2.default.mkdirSync(graniteDirectoryPath, { recursive: true });
    import_fs2.default.writeFileSync(brickModuleCompatPath, reverseBrickModuleCompatContent, "utf8");
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error);
    throw new Error(
      [
...
function reactCompatibilityPlugin(options) {
  ...
  const localDir = (0, import_utils.getLocalTempDirectoryPath)(rootDir);
  const compatScriptPath = import_path6.default.join(localDir, "react-19-compat.js");
  const compatScript = getReactCompatScript();
  import_fs3.default.writeFileSync(compatScriptPath, compatScript);
  return {
