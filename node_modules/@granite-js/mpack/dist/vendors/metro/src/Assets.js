...
async function getAssetData(assetPath, localPath, assetDataPlugins, platform = null, publicPath) {
  let assetUrlPath = localPath.startsWith("..") ? publicPath.replace(/\/$/, "") + "/" + path.dirname(localPath) : path.join(publicPath, path.dirname(localPath));
  if (path.sep === "\\") {
  ...
  const assetData = {
    ...
    width: dimensions ? dimensions.width / scale : void 0,
    height: dimensions ? dimensions.height / scale : void 0,
