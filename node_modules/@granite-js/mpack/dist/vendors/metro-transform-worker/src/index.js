...
const minifyCode = async (config, projectRoot, filename, code, source, map, reserved = []) => {
  const sourceMap = fromRawMappings([{ code, source, map, functionMap: null, path: filename }]).toMap(void 0, {});
  ...
  if (file.type === "js/script") {
    ...
    try {
      const opts = {
        ...
        dependencyTransformer: config.unstable_disableModuleWrapping === true ? disabledDependencyTransformer : void 0,
...
async function transformJSON(file, { options, config, projectRoot }) {
  let code = config.unstable_disableModuleWrapping === true ? JsFileWrapping.jsonToCommonJS(file.code) : JsFileWrapping.wrapJson(file.code, config.globalPrefix);
  let map = [];
  const minify = options.minify && options.unstable_transformProfile !== "hermes-canary" && options.unstable_transformProfile !== "hermes-stable";
  if (minify) {
...
module.exports = {
  transform: async (config, projectRoot, filename, data, options) => {
    ...
    if (unstable_dependencyMapReservedName != null) {
      ...
      if (position > -1) {
        throw new SyntaxError(
          "Source code contains the reserved string `" + unstable_dependencyMapReservedName + "` at character offset " + position
        );
      }
    }
    ...
    if (options.type === "asset") {
      ...
      return await transformAsset(file2, context);
    }
    const file = {
      filename,
      inputFileSize: data.length,
